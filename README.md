...
> **Note:** This is a fork of [djspiewak/sbt-github-packages](https://github.com/djspiewak/sbt-github-packages), maintained to add support for **sbt 2.x** (in addition to sbt 1.x). It is published under the `io.github.tayvs` organization. If you only need sbt 1.x, the original plugin works too.
...
```sbt
addSbtPlugin("io.github.tayvs" % "sbt-github-packages" % "<version>") // 0.5.6 is the latest version
```

Published for sbt 1.x and 2.x. No dependencies.
...
> **Tip:** sbt does not read `.env` files out of the box, so a `GITHUB_TOKEN` defined only in a project-local `.env` will not be visible to the `Environment` token source. If you'd like to keep your token in a `.env` file, add the [sbt-dotenv](https://github.com/Philippus/sbt-dotenv) plugin, which loads the file's entries as environment variables before settings are evaluated. With it in place, `TokenSource.Environment("GITHUB_TOKEN")` will pick up the token from your `.env`.
...
As an example, the documentation claims that, if you aren't publishing, your token only requires the `read:packages` grant (and *not* `write:packages`). Based on testing, as of right now, that appears to be false: `write:packages` is required for *all* API calls against GitHub Packages, including resolution.
...
Please, for the love of all that is holy, do not check this into your repository if you hard-code your credentials in this way. The token is a password. Treat it as such. A better approach would be to place the above into some global location, like `~/.sbt/1.0/github.sbt` (or `~/.sbt/2.0/github.sbt` for sbt 2.x).
...
You don't strictly *need* this plugin to consume artifacts from GitHub Packages. Since GitHub Packages speaks plain Maven, you can configure a resolver and credentials by hand:

```sbt
resolvers += "GitHub Package Registry" at "https://maven.pkg.github.com/OWNER/_"

credentials += Credentials(
  "GitHub Package Registry",     // realm — must match exactly
  "maven.pkg.github.com",        // host
  "_",                           // username — ignored by GitHub, so anything works
  sys.env.getOrElse("GITHUB_TOKEN", "")
)
```

This is correct and works fine. A few notes:

- The realm string (`"GitHub Package Registry"`) and host (`maven.pkg.github.com`) **must** match exactly, otherwise sbt won't attach the credentials to the request and you'll get a `401`/`not found`.
- The username is ignored by GitHub when a valid token is supplied, so `"_"` is a fine placeholder (this is exactly what the plugin uses internally).
- Using `"_"` as the repository in the URL resolves across *any* repository owned by `OWNER`, the same as `Resolver.githubPackages("OWNER")`.

What this plugin adds on top of the manual approach:

- `Resolver.githubPackages("OWNER")` convenience syntax instead of hand-writing the URL.
- A flexible `githubTokenSource` (environment variables, `~/.gitconfig`, and `||` fallback chains) rather than reading a single env var.
- *Publication* settings (`publishTo`, `homepage`, `scmInfo`, Ivy-style guard rails), which the manual resolver snippet above does **not** cover.

So if all you need is to *resolve* a few packages, the manual snippet is a perfectly good, dependency-free option. The plugin mainly pays off when you also publish or want the convenience/token-source flexibility.
